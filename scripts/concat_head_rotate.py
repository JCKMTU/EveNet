...
for session in sessions:
    if not os.path.exists(retargeted_dir + '/' + session + '/'):
        ...
        for file in head_files:
            ...
            # Assertion is that both opened files have same row counts.
            ...
            # In each frame change eular angles to quaternions and concat with rotated data.
            for frame in range(head_frames):
                ...
                # Scale the rotated data down by 100
                row_data = rota_data[frame] * .01

                # Still above 1.0 then minimize to 1.0
                row_data = np.minimum(1.0, row_data)

                # Cull unnecessary blendshapes
                row_data = np.delete(row_data, np.s_[13:27], axis=0)

                # Append facial mocap data to head rotation movement.
